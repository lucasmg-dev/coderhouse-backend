...
export abstract class BaseRepository<T> implements IWrite<T>, IRead<T> {
  ...
  async create(item: T): Promise<boolean> {
    const result: InsertOneWriteOpResult<WithId<T>> = await this._collection.insertOne(item);
  ...
  async update(id: string, item: T): Promise<boolean> {
    const result: UpdateWriteOpResult = await this._collection.updateOne({_id:new ObjectId(id)}, {$set: item});
    //console.log(result)
  ...
  async findOne(id: string): Promise<T> {
    const itemFound: T = await this._collection.findOne({_id:new ObjectId(id)})
    return itemFound;
  }
}